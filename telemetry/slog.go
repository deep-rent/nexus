...
// Attribute keys under which [Wrap] records the trace context.
...
// handler decorates an [slog.Handler] with trace correlation.
type handler struct {
	next slog.Handler
}

// Wrap wraps a handler so that every record logged with a context
...
//
//	logger := slog.New(telemetry.Wrap(log.NewHandler(
