// Copyright (c) 2025-present deep.rent GmbH (https://deep.rent)
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package sync

import (
	"context"
	"fmt"

	"github.com/deep-rent/nexus/internal/graph"
)

// Feeder coordinates the generation of the outbound synchronization Feed.
type Feeder[Tx any] struct {
	store    Store[Tx]
	graph    *graph.Graph[Entity]
	fetchers map[Entity]Fetcher[Tx]
	hlc      func() uint64 // Function to get current HLC time
}

// NewFeeder initializes a new Feed generator.
func NewFeeder[Tx any](store Store[Tx], hlc func() uint64) *Feeder[Tx] {
	return &Feeder[Tx]{
		store:    store,
		graph:    graph.New[Entity](),
		fetchers: make(map[Entity]Fetcher[Tx]),
		hlc:      hlc,
	}
}

// Register adds an entity fetcher to the Feeder. The dependencies must represent
// parent entities that this entity depends on (e.g. "meter" depends on "protocol").
func (f *Feeder[Tx]) Register(fetcher Fetcher[Tx], dependsOn ...Entity) {
	entity := fetcher.Entity()
	if _, exists := f.fetchers[entity]; exists {
		panic(fmt.Sprintf("sync: fetcher already registered for entity %q", entity))
	}

	f.fetchers[entity] = fetcher
	f.graph.AddNode(entity)

	for _, parent := range dependsOn {
		f.graph.AddEdge(entity, parent)
	}
}

// Build generates the complete synchronization payload for a client that last
// synced at the given `since` timestamp.
func (f *Feeder[Tx]) Build(ctx context.Context, since uint64) (*Feed, error) {
	// 1. Get topological sort (parents first, then children)
	sortedEntities, err := f.graph.Sort()
	if err != nil {
		return nil, fmt.Errorf("sync: failed to sort entities: %w", err)
	}

	// 2. Determine the new anchor time for the client
	newAnchor := f.hlc()

	var diffs []Diff

	// 3. Open a read-only transaction (represented by store.Exec)
	err = f.store.Exec(ctx, func(ctx context.Context, tx Tx) error {
		// We collect the fetched data so we only query once per entity
		deletesMap := make(map[Entity][]UUID)
		updatesMap := make(map[Entity]any)

		for _, entity := range sortedEntities {
			fetcher, exists := f.fetchers[entity]
			if !exists {
				// The entity might be purely a structural node in the DAG without a fetcher,
				// though normally it should have one.
				continue
			}

			updates, deletes, err := fetcher.Fetch(ctx, tx, since)
			if err != nil {
				return fmt.Errorf("failed to fetch %q: %w", entity, err)
			}

			if len(deletes) > 0 {
				deletesMap[entity] = deletes
			}
			// We check if updates is not nil (as any)
			if updates != nil {
				updatesMap[entity] = updates
			}
		}

		// Phase 1: Deletions. Must be applied bottom-up (children first).
		// We iterate the sorted list in reverse order.
		for i := len(sortedEntities) - 1; i >= 0; i-- {
			entity := sortedEntities[i]
			if deletes, ok := deletesMap[entity]; ok {
				diffs = append(diffs, Diff{
					Entity: entity,
					Delete: deletes,
					Update: nil,
				})
			}
		}

		// Phase 2: Updates. Must be applied top-down (parents first).
		// We iterate the sorted list in forward order.
		for _, entity := range sortedEntities {
			if updates, ok := updatesMap[entity]; ok {
				diffs = append(diffs, Diff{
					Entity: entity,
					Delete: nil,
					Update: updates,
				})
			}
		}

		return nil
	})
	if err != nil {
		return nil, err
	}

	return &Feed{
		Time: newAnchor,
		Diff: diffs,
	}, nil
}
