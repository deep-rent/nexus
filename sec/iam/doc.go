...
// Package iam implements an identity and access management server: an OAuth
// 2.0 authorization server combined with the first-party login machinery
// around it — password and passwordless logins, multi-step authentication
// flows, WebAuthn passkeys, social login, and remember-me device trust.
//
// # Architecture
//
...
// and initialize a [Server] using the desired [oauth.Grant] types as options.
//
// Example:
//
...
//	  iam.WithIdentityProvider("google", myGoogleProvider),
//	)
//
...
// with the [github.com/deep-rent/nexus/sec/iam/otp] helpers ([otp.ViaText],
...
//	  iam.WithGrant(grant.AuthCode()),
//	  iam.WithFlow(func(
//	    ctx context.Context, sub iam.Subject, dev trust.Device, b iam.Steps,
//	  ) ([]flow.Step, error) {
//	    if dev.Trusted {
