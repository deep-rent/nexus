...
func (s otpStore) Get(
	...
	oc, err := s.sessions.GetOTPChallenge(ctx, oauth.Digest(id))
	if err != nil {
...
func (s otpStore) Delete(ctx context.Context, id string) (bool, error) {
	return s.sessions.DeleteOTPChallenge(ctx, oauth.Digest(id))
}
...
func (s otpStore) toChallenge(c otp.Challenge) (OTPChallenge, error) {
	...
	return OTPChallenge{
		Challenge: oauth.Digest(c.ID),
		SubjectID: sid,
		Code:      oauth.Digest(c.Code),
		Attempts:  c.Attempts,
		Resends:   c.Resends,
...
func (s flowStore) Get(
	...
	ft, err := s.sessions.GetFlowTransaction(ctx, oauth.Digest(id))
	if err != nil {
...
func (s flowStore) Delete(ctx context.Context, id string) (bool, error) {
	return s.sessions.DeleteFlowTransaction(ctx, oauth.Digest(id))
}
