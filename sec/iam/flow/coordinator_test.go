...
func TestCoordinator_CustomHasher(t *testing.T) {
	t.Parallel()

	store := newMemStore()
	hasher := digest.New(nil) // fresh hasher instance, default algorithm
	c := flow.New(store, flow.WithHasher(hasher))

	step := &fakeStep{id: "otp", verdict: flow.VerdictOK}
	handle, _, err := c.Begin(t.Context(), "user-1", false, []flow.Step{step})
	if err != nil {
		t.Fatalf("Begin: %v", err)
	}

	// The transaction must be keyed by the injected hasher's digest, and the
	// same hasher must resolve it on continuation.
	if _, found, _ := store.Get(
		t.Context(), hasher.String(handle),
	); !found {
		t.Fatal("transaction not stored under the injected hasher's digest")
	}
	res, err := c.Continue(t.Context(), handle, planOf(step), code)
	if err != nil {
		t.Fatalf("Continue: %v", err)
	}
	if res.Status != flow.StatusDone {
		t.Errorf("got status %v; want done", res.Status)
	}
}
