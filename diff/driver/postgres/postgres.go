...
//	diff.Register[*sql.Tx, File](reg, "file", files,
//	    diff.WithOwner("asset", "asset_id"))
//	diff.RegisterShares(reg, store.Shares())
//
...
// DDL statements matching the default configuration. Feed them to the
// migrate package instead of calling [Store.Init] when schema changes are
// managed through migrations. They must be kept in sync with the statements
// generated by [Store.Init].
const (
	// DDLSequence creates the global sequence ordering the patch feed.
	DDLSequence = `
CREATE SEQUENCE IF NOT EXISTS "public"."diff_seq" CACHE 1;`

	// DDLMutations creates the mutation deduplication table.
	DDLMutations = `
...
CREATE INDEX IF NOT EXISTS diff_mutations_applied_at
  ON "public"."diff_mutations" (applied_at);`
	...
	// DDLTombstones creates the tombstone table recording deletions across
	// all entity types.
	DDLTombstones = `
...
CREATE INDEX IF NOT EXISTS diff_tombstones_deleted_at
  ON "public"."diff_tombstones" (deleted_at);`
	...
	// DDLState creates the state table and seeds the retention floor.
	DDLState = `
...
INSERT INTO "public"."diff_state" (key, seq)
  VALUES ('floor', 0) ON CONFLICT DO NOTHING;`
	...
	// DDLShares creates the table backing the reserved "share" entity type.
	DDLShares = `
...
// Init ensures that all bookkeeping objects exist in the target schema. All
// statements are idempotent, so Init may be called on every startup.
// Alternatively, feed the exported DDL constants to the migrate package.
...
// ddl assembles the idempotent bootstrap statements for the configured
// object names. It must be kept in sync with the exported DDL constants.
func (s *Store) ddl() []string {
	return []string{
		"CREATE SEQUENCE IF NOT EXISTS " + s.identSequence + " CACHE 1",

		"CREATE TABLE IF NOT EXISTS " + s.identMutations + " (" +
			"id UUID PRIMARY KEY, " +
			"user_id UUID NOT NULL, " +
			"applied_at TIMESTAMPTZ NOT NULL DEFAULT now())",
		"CREATE INDEX IF NOT EXISTS " + escape(s.mutations+"_applied_at") +
			" ON " + s.identMutations + " (applied_at)",

		"CREATE TABLE IF NOT EXISTS " + s.identTombstones + " (" +
			"type TEXT NOT NULL, " +
			"id UUID NOT NULL, " +
			"user_id UUID NOT NULL, " +
			"team_id UUID, " +
			...
			"deleted_at TIMESTAMPTZ NOT NULL DEFAULT now(), " +
			"PRIMARY KEY (type, id))",
		"CREATE INDEX IF NOT EXISTS " + escape(s.tombstones+"_user_seq") +
			" ON " + s.identTombstones + " (user_id, seq)",
		"CREATE INDEX IF NOT EXISTS " + escape(s.tombstones+"_team_seq") +
			" ON " + s.identTombstones + " (team_id, seq)" +
			" WHERE team_id IS NOT NULL",
		"CREATE INDEX IF NOT EXISTS " + escape(s.tombstones+"_deleted_at") +
			" ON " + s.identTombstones + " (deleted_at)",

		"CREATE TABLE IF NOT EXISTS " + s.identState + " (" +
			"key TEXT PRIMARY KEY, " +
			"seq BIGINT NOT NULL)",
		"INSERT INTO " + s.identState + " (key, seq) VALUES ('floor', 0)" +
			" ON CONFLICT DO NOTHING",

		"CREATE TABLE IF NOT EXISTS " + s.identShares + " (" +
			"id UUID PRIMARY KEY, " +
			"user_id UUID NOT NULL, " +
			"team_id UUID NOT NULL, " +
			"hlc BIGINT NOT NULL, " +
			"seq BIGINT NOT NULL, " +
			"UNIQUE (user_id, team_id))",
		"CREATE INDEX IF NOT EXISTS " + escape(s.shares+"_user_seq") +
			" ON " + s.identShares + " (user_id, seq)",
		"CREATE INDEX IF NOT EXISTS " + escape(s.shares+"_team_seq") +
			" ON " + s.identShares + " (team_id, seq)",
	}
}
...
func (s *Store) Lock(ctx context.Context, tx *sql.Tx, keys []uuid.UUID) error {
	if len(keys) == 0 {
		return nil
	}
...
// lockKey derives a positive 64-bit advisory lock key from a scope UUID.
...
func (s *Store) Claim(
	...
	rows, err := tx.QueryContext(ctx, query, toStrings(ids), userID.String())
	if err != nil {
...
// backed by the share grants table. Register it via [diff.RegisterShares].
...
func (h *shares) Upsert(
	...
	for _, op := range ops {
		...
		args := []any{
			op.Meta.ID.String(),
			op.Meta.UserID.String(),
			op.Meta.TeamID.String(),
			int64(op.Time),
			scope.UserID.String(),
			diff.TypeShare,
		}
...
func resolve(
	...
	for rows.Next() {
		var rawID, rawUser string
		var rawTeam sql.NullString
		if err := rows.Scan(&rawID, &rawUser, &rawTeam); err != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to parse document owner: %w", err)
		}
		meta := diff.Meta{ID: id, UserID: user}
		if rawTeam.Valid {
			team, err := uuid.Parse(rawTeam.String)
			if err != nil {
				return nil, fmt.Errorf("failed to parse document team: %w", err)
			}
			meta.TeamID = &team
		}
		out[id] = meta
...
func teamString(team *uuid.UUID) string {
	if team == nil {
		return ""
	}
	return team.String()
