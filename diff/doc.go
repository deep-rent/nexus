...
//
//  1. Floor(1) rejects a cursor that already predates the pruning floor.
//  2. Barrier consumes one sequence value up front. Every row written by
...
//     upper bound.
//  3. The writes apply, each taking sequence values above the barrier.
//  4. Fetch returns rows and tombstones with since < seq < barrier, the
//     work the client missed, excluding this request's own writes.
//  5. When the page is not full (More is false), the cursor advances to the
...
//  6. Floor(2) re-reads the floor after the scan: tombstone pruning runs
