...
// Handler returns a [router.HandlerFunc] that serves the vault's public keys
// as a standard JSON Web Key Set (JWKS) document.
//
// This allows other services to dynamically fetch the public keys required
// to verify signatures generated by this vault.
func Handler(v Vault) router.HandlerFunc {
	return func(e *router.Exchange) error {
		data, err := jwk.WriteSet(v.Keys())
		if err != nil {
			return err
		}
